...
export default function DashboardClient(){
 ...
 const visible=modules.filter(m=>m.permissions.length===0||m.permissions.some(p=>me.permissions.includes(p)));const parent=me.roles.includes("PARENT");
 ...
 <section className="workspace"><header><div><p className="eyebrow">{roleLabel(me.roles)}</p><h1>{parent?"Parent Dashboard":"Dashboard"}</h1><p className="muted">{me.email}</p></div><div className="header-actions"><span className="status-pill">v1.1</span><LogoutButton/></div></header>
