...
from app.core.auth import current_user,permission_codes,has_role,linked_student_ids,require_linked_student,scoped_student_id,require_self_student
...
def payment(p:PaymentIn,u:User=Depends(current_user),db:Session=Depends(get_db)):
 ...
 if inv: require_linked_student(db,u,inv.student_id);require_self_student(db,u,inv.student_id)
...
class WebhookIn(BaseModel):
 tenant_id:UUID;provider:str;provider_order_id:str;payment_reference:str;status:str
...
async def payment_webhook(p:WebhookIn,request:Request,x_payment_signature:str|None=Header(default=None),db:Session=Depends(get_db)):
 ...
 order=db.scalar(select(PaymentOrder).where(PaymentOrder.tenant_id==p.tenant_id,PaymentOrder.provider==p.provider,PaymentOrder.provider_order_id==p.provider_order_id))
 ...
 payment=Payment(tenant_id=order.tenant_id,invoice_id=inv.id,reference=p.payment_reference,amount=amount,method=order.provider,status="CONFIRMED");db.add(payment);inv.paid_amount+=amount;inv.status="PAID" if inv.paid_amount==inv.amount else "PARTIALLY_PAID";order.status="CONFIRMED";db.flush();db.add(AuditEvent(tenant_id=order.tenant_id,user_id=None,action="finance.payment_webhook.confirmed",resource_type="payment_order",resource_id=str(order.id),request_id=request.state.request_id));db.commit()
